...
public class PostApiController {
    ...
    public ResponseDto<?> detail(@PathVariable Integer id) {
        ...
        return new ResponseDto<>(1, "성공", detailResponseDto); // comment 생성된다. → MessageConverter
    }

    @GetMapping("/api/post")
    public ResponseDto<?> list(Integer page) {
        Page<Post> posts = postService.글목록(page);
        // 응답의 DTO를 만들어서 ← posts를 옮긴다.(라이브러리 있음)
    ...
    public ResponseDto<?> write(@RequestBody WriteDto writeDto) {
        ...
        // 원래는 그냥 dto 바로 넘겼는데, 지금 dto를 넘기면 session값 두개 넘겨야해서 하나로 합쳐서 넘긴다.
